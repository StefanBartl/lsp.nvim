...
--- exactly what happened to the fallback separator of the winbar breadcrumb --
--- the string the user sees between two symbols -- and it was found by reading,
--- not by any check. The tools that cause it are the ones a Windows session
--- reaches for: PowerShell's default output encoding, a script that opens a file
...
local SIGNATURES = {
  ...
  {
    -- Three- and four-byte characters (the typographic quotes and arrows, emoji)
    -- read as Latin-1: the lead byte becomes a lowercase a with circumflex or an
    -- eth, and the rest keeps its value as U+0080..U+00BF.
...
--- The working directory, which is the repository root: `TESTS/minimal_init.lua`
...
describe("the repository's text files", function()
  ...
  it("recognises the damage the patterns are written for", function()
    -- U+203A (the single right angle quote) is E2 80 BA; read as Latin-1 and
    -- written back as UTF-8 it becomes C3 A2, C2 80, C2 BA.
    local latin1 = "\195\162\194\128\194\186"
    -- The same character read as Windows-1252: C3 A2, E2 82 AC, C2 BA.
    ...
    -- And the real characters are left alone.
    local clean = "\226\128\186 \195\164 \195\162"
