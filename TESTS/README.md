...
| `gitsigns_actions_spec.lua` | `core/gitsigns_actions`, with a **real** `vim.lsp.start` on the in-process server: the hunk-overlap rules (including a deletion above the first line and after the last, which gitsigns bends), the kind the code-action indicator does not light on, that no request is left pending on Neovim's client, and that the commands run. |
