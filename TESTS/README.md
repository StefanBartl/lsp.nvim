...
| `peek_spec.lua` | `core/peek` against real windows and real temp files: what an answer is flattened to, the float, the keys given back (including a buffer-local map they shadowed -- to its own buffer, also when the peek is closed from another window), unloading a buffer the peek loaded, that a buffer taken into a window or edited beyond unloading is listed (`bufadd` leaves it out of `:ls`), stacking, and every way of taking a peek into a real window. |
| `implement_spec.lua` | `core/implement`: which symbols it asks about, that it sends nothing without a server that implements the method, the `max_requests` cap, one round per `changedtick`, one debounce per buffer, that an answer for text that has since changed is not acted on -- neither the symbols nor, once the requests are out, the `implementation` answers, which must not clear or move the markers already drawn -- and that a `text` that cannot print a count falls back. |
| `gitsigns_actions_spec.lua` | `core/gitsigns_actions`, with a **real** `vim.lsp.start` on the in-process server: the hunk-overlap rules (including a deletion above the first line and after the last, which gitsigns bends), the kind the code-action indicator does not light on, that no request is left pending on Neovim's client, that the commands run, that they act on the selection they were offered for and not on the hunk the cursor ended in (with the request at the cursor carrying no range, or a one-line range would stage a line instead of the hunk), and that a force-stopped client reports its exit -- it otherwise stays listed as stopped and is never attached again. |
| `internal_clients_spec.lua` | `lsp.core.util`'s split between lsp.nvim's own in-process clients (`lsp.nvim-*`) and language servers, which the winbar guard, `:Lsp stop` / `:Lsp restart` and the stop completion rely on. |
| `source_encoding_spec.lua` | The repository's `lua/`, `TESTS/`, `docs/`, `doc/`, `scripts/` and `README.md` for double-encoded UTF-8 -- text read as Latin-1 or Windows-1252 and written back as UTF-8, which Lua, stylua and a reviewer all let through. It found the fallback separator of the winbar breadcrumb. |
