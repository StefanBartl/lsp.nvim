...
| `gitsigns_actions_spec.lua` | `core/gitsigns_actions`, with a **real** `vim.lsp.start` on the in-process server: the hunk-overlap rules (including a deletion above the first line and after the last, which gitsigns bends), the kind the code-action indicator does not light on, that no request is left pending on Neovim's client, that the commands run, and that a force-stopped client reports its exit -- it otherwise stays listed as stopped and is never attached again. |
